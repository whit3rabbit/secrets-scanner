...
description: Install, remove, hook, or run secrets-scanner to detect leaked API keys and credentials in repos or staged commits.
homepage: https://github.com/whit3rabbit/secrets-scanner
metadata: {"openclaw":{"homepage":"https://github.com/whit3rabbit/secrets-scanner","install":[{"id":"brew","kind":"brew","formula":"whit3rabbit/tap/secrets-scanner","bins":["secrets-scanner"],"label":"Install secrets-scanner (Homebrew)"}]}}
---
...
1. **Native git hook** (no framework). Run inside the target repo:
   ```sh
   bash "{baseDir}/scripts/install-git-hook.sh"
   ```
   ...
   `bash "{baseDir}/scripts/uninstall-git-hook.sh"`.
...
- Scripts: `{baseDir}/scripts/install.sh`, `{baseDir}/scripts/uninstall.sh`, `{baseDir}/scripts/install-git-hook.sh`, `{baseDir}/scripts/uninstall-git-hook.sh`
