...
- Many local rules use look-around (`(?<!…)`/`(?!…)`) which Rust's `regex` rejects, so those rules are disabled — raw rule count ≫ compiled/active count (e.g. lean 382 raw → 233 active). `tests/scan_integration.rs` snapshots the compiled count.
...
The default lean build embeds gitleaks + local (`local` priority 100 > `gitleaks` 10, preserving "local overrides upstream"). `secrets-patterns-db` (`embed = false`) is opt-in via `--features full-ruleset`; `kingfisher` is declared but deferred (YAML, no converter, skipped). Changing the manifest or any listed source file triggers a recompile. If `assets/sources.toml` is absent, build.rs falls back to the legacy gitleaks+local 2-way merge.
